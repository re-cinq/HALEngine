...
**`re-lint/prefer-polymorphism` on `createProvider`** (`src/providers/providerFactory.ts`). The five-way `switch` on `config.type` is the lazy-loading boundary: each arm `require`s its provider's module only when that provider is selected, so a consumer who picks Vertex never needs the Bedrock SDK installed. A lookup table replaces the dispatch only if its values are thunks; built the obvious way, with the modules as values, every SDK loads on module evaluation - which is the failure commit `0fb475c` fixed. The dispatch is doing work the table would have to reproduce, so replacing it buys nothing and risks that regression.
