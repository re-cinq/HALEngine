...
Any relative path with a file extension followed by `#L<n>`, in any spec under `specs/<slug>/spec.md` plus `.specify/spec.md`. A link whose fragment is not a line number is not an anchor and is left alone, however much the file around it moves ([validated by: leaves a link whose fragment is not a line number untouched](../../scripts/repoint-spec-anchors.test.ts#L506)). Test files are the common case, but nothing restricts it to them: a citation into a workflow, a config or a script is repointed the same way ([validated by: repoints docs, README and script citations alike](../../scripts/repoint-spec-anchors.test.ts#L454), [root config](../../scripts/repoint-spec-anchors.test.ts#L492), [workflow and manifest](../../scripts/repoint-spec-anchors.test.ts#L506)).
...
- the line is blank, or holds only closing punctuation — `)`, `]`, `}`, `>`, `,`, `;` ([validated by: fails in both modes when the anchor lands on a blank line](../../scripts/repoint-spec-anchors.test.ts#L263), [closing punctuation](../../scripts/repoint-spec-anchors.test.ts#L287)).
...
- The declaration itself passes, and so does the enclosing `describe` — citing a group is a legitimate reference ([validated by: accepts an anchor on the it declaration itself](../../scripts/repoint-spec-anchors.test.ts#L533), [describe](../../scripts/repoint-spec-anchors.test.ts#L566)).
