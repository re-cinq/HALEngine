...
- The `v0.2.0` release is created after the bootstrap, on the commit it published, so the changelog's version link resolves. `publish.yml` runs on it and fails at the staging step against a version that already exists; that one red run publishes nothing and is expected. The attestation on `0.2.0` names the bootstrap workflow, which no longer exists in the tree, and `npm audit signatures` still verifies it, because the attestation records the workflow at the commit it ran from.
