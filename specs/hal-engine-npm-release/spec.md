...
- A `v*` tag triggers one publish job. There is no `workflow_dispatch` trigger: a publish that can be started by hand is a publish whose input is a branch somebody chose in a dropdown, and the tag is the only thing the version guard and the ancestry check can be stated against. The one exception is the single-use bootstrap under § First publish, which is a separate workflow file, runs once, and is deleted in the same change that registers the trusted publisher.
...
- A `v0.2.1` tag then stages with no secret present in the repository, and is approved the same way. That is the evidence this step produces. `0.2.1` carries its own `## [0.2.1]` heading in the changelog, because `check-version.sh` requires one, and its entry says what it is: the first tokenless publish, with no change to the package.
- The `v0.2.0` tag is pushed after the bootstrap, on the commit it published, so the changelog's version link resolves. `publish.yml` runs on that tag and fails at `npm publish` against a version that already exists; that one red run publishes nothing and is expected. The attestation on `0.2.0` names the bootstrap workflow, which no longer exists in the tree, and `npm audit signatures` still verifies it, because the attestation records the workflow at the commit it ran from.
