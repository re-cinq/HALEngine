...
- A `v0.2.1` tag then publishes with no secret present in the repository. That is the evidence this step produces. `0.2.1` carries its own `## [0.2.1]` heading in the changelog, because `check-version.sh` requires one, and its entry says what it is: the first tokenless publish, with no change to the package.
