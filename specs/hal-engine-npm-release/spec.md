...
- A single-use, manually triggered workflow stages `0.2.0` once with a short-lived token, and a maintainer approves the stage.
...
- Everything here happens once and cannot be redone: an approved version cannot be replaced, and after 72 hours cannot be withdrawn. Staging moves the point of no return from the tag push to the approval, which is the only reason there is a gap to reconsider in at all. It therefore runs last, after the tarball smoke test passes and after the README that becomes the package's front page is correct.
