...
- `afterModelResponse` is called once per user message after the round loop exits ([chatOrchestrator.ts#L125](../../src/orchestration/chatOrchestrator.ts#L125)), and is passed the `lastUsage` local — the most recent round's usage, not a sum across tool rounds.
