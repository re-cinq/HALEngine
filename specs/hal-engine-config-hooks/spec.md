...
- `afterModelResponse` is called once per user message after the round loop exits ([chatOrchestrator.ts#L125](../../src/orchestration/chatOrchestrator.ts#L125)), and is passed `totalUsage` — the accumulated sum of usage across every provider call in the turn ([validated by: sums the usage of every round, not only the last](../../src/orchestration/chatOrchestrator.test.ts#L528)).
