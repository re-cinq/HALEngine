...
- The chat is looked up second. An id with no chat is `404 Chat not found` ([validated by: answers 404 for a chat that does not exist](../../src/transport/routes/chats.test.ts#L70)).
- Ownership is checked third. A caller who is not the recorded owner gets `403 Forbidden` and no part of the chat ([validated by: refuses a different user with 403 and no chat content](../../src/transport/routes/chats.test.ts#L61)).
- A caller who is not the owner still sees `404` for an id that does not exist, so ownership never becomes an oracle for which ids are real ([validated by: answers 404 before 403, so a wrong owner cannot probe which ids exist](../../src/transport/routes/chats.test.ts#L78)).
- Owner ids are compared strictly, so numeric `1` and string `"1"` are different users rather than the same one ([validated by: compares owner ids strictly, so numeric 1 and string "1" are different users](../../src/transport/routes/chats.test.ts#L96)).
...
- Creating a chat is refused `401 Unauthorized` ([validated by: refuses to create a chat](../../src/transport/routes/chats.test.ts#L206)).
- Reading a chat is refused the same way ([validated by: refuses to read a chat](../../src/transport/routes/chats.test.ts#L214)).
- A message is refused and the orchestrator is never invoked, so an unauthenticated request costs no model call ([validated by: refuses to post a message, and never reaches the orchestrator](../../src/transport/routes/chats.test.ts#L222)).
- The `401` precedes the chat lookup, so an unauthenticated caller cannot probe which ids exist ([validated by: answers 401 before 404, so an unauthenticated caller cannot probe which chat ids exist](../../src/transport/routes/chats.test.ts#L234)).
...
- Middleware that runs but attaches no `user` is refused `401`, and the chat it asked for is not revealed ([validated by: refuses a request whose middleware attached no user, without revealing the chat](../../src/transport/routes/chats.test.ts#L87)).
- A `user` carrying no `id` is refused ([validated by: refuses a user carrying no id](../../src/transport/routes/chats.test.ts#L265)).
- A `null` id is refused ([validated by: refuses a null id](../../src/transport/routes/chats.test.ts#L273)).
- An empty-string id is refused ([validated by: refuses an empty-string id](../../src/transport/routes/chats.test.ts#L257)).
- The numeric id `0` is accepted: `id` is `string | number`, so `0` is a legal id even though it is falsy, and the check is by type and emptiness rather than by truthiness ([validated by: accepts the numeric id 0, which is falsy but legal](../../src/transport/routes/chats.test.ts#L248)).
- A refused caller leaves nothing behind - no chat is recorded for it ([validated by: never records a chat for a caller it refused](../../src/transport/routes/chats.test.ts#L281)).
...
- A request refused because no middleware is configured emits exactly one `warn` on category `http` ([validated by: emits exactly one warn on category http when no middleware is configured](../../src/transport/routes/chats.test.ts#L318)).
- A request refused because the middleware attached no usable user emits exactly one ([validated by: emits exactly one warn when the middleware attaches no usable user](../../src/transport/routes/chats.test.ts#L331)).
- Nothing request-derived appears in the emitted data ([validated by: records nothing derived from the request, so a refusal cannot leak what was asked for](../../src/transport/routes/chats.test.ts#L339)).
- An authorised request emits no warning at all, so the line means a refusal rather than traffic ([validated by: says nothing at all when the request is authorised](../../src/transport/routes/chats.test.ts#L347)).
...
- A created chat is answered `201` with a body carrying exactly `id` and `createdAt`, which is the pair a client destructures ([validated by: answers 201 with exactly an id and a createdAt](../../src/transport/routes/chats.test.ts#L108)).
- The creating user's `workspaceId` is stored on the chat and reaches the orchestrator on the next message, two requests later ([validated by: carries the creating user workspace through to the orchestrator](../../src/transport/routes/chats.test.ts#L120)).
...
- With no `authMiddleware`, `POST /hal/chats` is refused ([validated by: refuses to create a chat](../../src/transport/createApp.test.ts#L20)).
- Health still answers `200` alongside that refusal ([validated by: still answers health, so a denied chat route is not a broken app](../../src/transport/createApp.test.ts#L27)).
...
- An Express `RequestHandler` is accepted as an `HttpAuthMiddleware` ([validated by: accepts a plain express RequestHandler, which is what a consumer already has](../../src/transport/createApp.test.ts#L64)).
- An `HttpAuthMiddleware` is usable as an Express `RequestHandler` ([validated by: is itself usable as an express RequestHandler, which is how the router mounts it](../../src/transport/createApp.test.ts#L71)).
...
Returns the chat's id, messages and creation time to its owner ([validated by: serves the chat to the user who created it](../../src/transport/routes/chats.test.ts#L52)).

## POST /chats/:id/messages

- The owner's message is accepted and answered `201` ([validated by: accepts a message from the user who created the chat](../../src/transport/routes/chats.test.ts#L134)).
- Another user's message is refused, and the orchestrator is never invoked, so a rejected request costs no model call ([validated by: refuses a different user and never reaches the orchestrator](../../src/transport/routes/chats.test.ts#L143)).
- A message to an id with no chat is `404`, by the same lookup-first order ([validated by: answers 404 when the chat a message names does not exist](../../src/transport/routes/chats.test.ts#L157)).
- Ownership is checked before the body is validated, so a wrong owner sending an empty body gets `403` rather than `400` ([validated by: refuses a different user before validating the body, so 403 beats 400](../../src/transport/routes/chats.test.ts#L165)).
