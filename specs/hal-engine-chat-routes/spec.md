...
- Creating a chat is refused `401 Unauthorized` ([validated by: refuses to create a chat](../../src/transport/routes/chats.test.ts#L168)).
- Reading a chat is refused the same way ([validated by: refuses to read a chat](../../src/transport/routes/chats.test.ts#L176)).
- A message is refused and the orchestrator is never invoked, so an unauthenticated request costs no model call ([validated by: refuses to post a message, and never reaches the orchestrator](../../src/transport/routes/chats.test.ts#L184)).
- The `401` precedes the chat lookup, so an unauthenticated caller cannot probe which ids exist ([validated by: answers 401 before 404, so an unauthenticated caller cannot probe which chat ids exist](../../src/transport/routes/chats.test.ts#L196)).
...
- A `user` carrying no `id` is refused ([validated by: refuses a user carrying no id](../../src/transport/routes/chats.test.ts#L227)).
- A `null` id is refused ([validated by: refuses a null id](../../src/transport/routes/chats.test.ts#L235)).
- An empty-string id is refused ([validated by: refuses an empty-string id](../../src/transport/routes/chats.test.ts#L219)).
- The numeric id `0` is accepted: `id` is `string | number`, so `0` is a legal id even though it is falsy, and the check is by type and emptiness rather than by truthiness ([validated by: accepts the numeric id 0, which is falsy but legal](../../src/transport/routes/chats.test.ts#L210)).
- A refused caller leaves nothing behind - no chat is recorded for it ([validated by: never records a chat for a caller it refused](../../src/transport/routes/chats.test.ts#L243)).
...
- A request refused because no middleware is configured emits exactly one `warn` on category `http` ([validated by: emits exactly one warn on category http when no middleware is configured](../../src/transport/routes/chats.test.ts#L280)).
- A request refused because the middleware attached no usable user emits exactly one ([validated by: emits exactly one warn when the middleware attaches no usable user](../../src/transport/routes/chats.test.ts#L293)).
- Nothing request-derived appears in the emitted data ([validated by: records nothing derived from the request, so a refusal cannot leak what was asked for](../../src/transport/routes/chats.test.ts#L301)).
- An authorised request emits no warning at all, so the line means a refusal rather than traffic ([validated by: says nothing at all when the request is authorised](../../src/transport/routes/chats.test.ts#L309)).
