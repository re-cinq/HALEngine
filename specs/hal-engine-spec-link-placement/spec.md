...
- A citation mid-way through a list item is misplaced, and each one is reported separately against the item's own first line ([validated by: a list item with two mid-item citations reports both at the item's line](../../scripts/check-spec-links.test.ts#L37)).
- A citation mid-way through a paragraph sentence is misplaced, and is reported against the line the paragraph starts on, not the line the link sits on ([validated by: a mid-sentence citation in a paragraph reports one finding at the paragraph's line](../../scripts/check-spec-links.test.ts#L44)).
- A citation closing its sentence is where the rule wants it and is never reported ([validated by: a paragraph sentence whose citation is trailing reports nothing](../../scripts/check-spec-links.test.ts#L61)).
- A mid-sentence link to something other than a test file - a script, a doc - is prose, not a miscited test, and is never reported ([validated by: a mid-sentence link to a script rather than a test reports nothing](../../scripts/check-spec-links.test.ts#L65)).
- A citation written inside backticks is documentation of the convention rather than a use of it, and is never reported ([validated by: a citation written inside backticks reports nothing](../../scripts/check-spec-links.test.ts#L69)).
