...
- [x] **wraps a non-Error rejection in an Error whose cause is the original and
...
- [x] Red: the new test asserts `onErrorCallCount: 1` / `isError` / `causeIsOriginal`
      for string, object and undefined; today all three read `0` / `false` because
      `error instanceof Error` skips `onError`.
- [x] Green: in the `catch`, when `hooks?.onError` is set, wrap a non-Error in
...
- [x] Refactor: existing `onError` and lifecycle-order tests stay green unedited.
- [x] Doc (implementer): CHANGELOG `[Unreleased]` entry saying what changes for
