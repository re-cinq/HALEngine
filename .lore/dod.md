...
- [x] **runs onError then afterSession when it throws, and still rejects to the caller** —
...
- [x] Move the `beforeSession` call inside the `try` block in
...
- [x] Confirm the non-throwing `beforeSession` path is unchanged — the existing
      hook tests (`called before anything else`, `full lifecycle order`) stay green
      without edits.
- [x] Criterion 5, owned by the implementer: expand the new guarantee bullet in
